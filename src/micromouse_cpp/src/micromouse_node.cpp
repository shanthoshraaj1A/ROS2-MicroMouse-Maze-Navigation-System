...
namespace micromouse {
  ...
  // Run navigation immediately (standalone MMS mode)
  void MicroMouseNode::run_navigation() {
  ...
  // Initialize internal wall storage and set perimeter walls
  void MicroMouseNode::init_walls() {
    walls_.assign(
        W_, std::vector<std::array<bool, 4>>(H_, {false, false, false, false}));

    // Set perimeter walls: Bottom and Top
    for (int x = 0; x < W_; ++x) {
     ...
     // Set perimeter walls: Left and Right
    for (int y = 0; y < H_; ++y) {
  ...
  // Mark wall on both sides (current cell and neighbor) 
  void MicroMouseNode::mark_wall(const Cell& c, Dir d) {
    if (!in_bounds(c))
      return;
  ...
  void MicroMouseNode::sense_and_update(const Cell& pos, Dir facing) {
  ...
  // Check if edge between cell and direction is free (no wall)
  bool MicroMouseNode::edge_free(const Cell& c, Dir d) const {
    if (!in_bounds(c))
      return false;
  ...
  // Turn robot to face desired direction
  void MicroMouseNode::face_direction(Dir want) {
  ...
  // Compute path using Depth-First Search
  std::optional<std::vector<Cell>> MicroMouseNode::dfs_plan(const Cell& start, const Cell& goal) {
  ...
  // Color the path to the goal in the maze
  void MicroMouseNode::color_path(const std::vector<Cell>& path, const Cell& goal) {
  ...
  // Execute navigation with dynamic replanning
  bool MicroMouseNode::execute_with_replanning(
  ...
  // Function to publish position
  void MicroMouseNode::publish_position() {
  ...
  // Get the elapsed seconds object
  double MicroMouseNode::get_elapsed_seconds() const {
  ...
  void MicroMouseNode::get_status_callback(
  ...
  rclcpp_action::GoalResponse MicroMouseNode::handle_goal(
  ...
  rclcpp_action::CancelResponse MicroMouseNode::handle_cancel(
  ...
  void MicroMouseNode::handle_accepted(const std::shared_ptr<GoalHandleNavigate> goal_handle) {
    std::thread{
  ...
  void MicroMouseNode::execute_action(const std::shared_ptr<GoalHandleNavigate> goal_handle) {
